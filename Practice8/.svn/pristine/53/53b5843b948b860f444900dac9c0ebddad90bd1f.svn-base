    ...
    public List<?> getUserTeams(User user) throws SQLException {
        ...
        for (int id : teams_id) {
            statement = connection.prepareStatement
                    ("SELECT TEAMS.NAME, TEAMS.TEAM_ID FROM TEAMS WHERE TEAMS.TEAM_ID = ?");
            statement.setInt(1, id);
            statement.execute();
            result = statement.executeQuery();
            result.next();
            Team team = Team.createTeam(result.getString(1));
            team.setId(result.getInt(2));
            teams.add(team);
        }
    ...
    public void updateTeam(Team team) {
        try {
            PreparedStatement statement =
                    connection.prepareStatement("UPDATE TEAMS SET NAME = ? WHERE TEAM_ID = ?");
